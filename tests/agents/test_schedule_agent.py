...
def make_agent(action_days=None, action_inputs=None):
    params = {}
    if action_days is not None or action_inputs is not None:
        params["agent"] = {}
        if action_days is not None:
            params["agent"]["action_days"] = action_days
        if action_inputs is not None:
            params["agent"]["action_inputs"] = action_inputs
...
class TestScheduleAgent:
    ...
    async def test_single_step_constant_ppfd(self):
        agent = make_agent(action_days=[0], action_inputs=[150.0])
        await agent.start(dt(START, 10))
    ...
    async def test_multi_step_schedule(self):
        # Days 0-4: 100, days 5-9: 150, days 10+: 200
    ...
    async def test_schedule_transition_on_exact_day(self):
        agent = make_agent(action_days=[0, 3], action_inputs=[80.0, 160.0])
        await agent.start(dt(START, 10))

        action_before, _ = await agent.step(0.0, dt(START + timedelta(days=2), 12), {})
        assert action_before == pytest.approx(80.0)

        action_on, _ = await agent.step(0.0, dt(START + timedelta(days=3), 12), {})
    ...
    async def test_start_and_step_return_same_action(self):
        agent = make_agent(action_days=[0], action_inputs=[120.0])
        obs = dt(START, 12)
