...
# %%
df = pd.read_csv('/data/online/E6/P3/data.csv')
...
for (name, group_df) in df.groupby("environment.zone"):
    ...
    path = Path(f"/data/online/E6/P3/Spreadsheet-{name}/z{name}")
    # images
    ...
    start_image = image_file_df['image_name'][0]
    image_file_df = image_file_df[1:]
