...
# nazmus_exp/<dataset>/
dataset_paths = chain(
    Path("phytochrome_exp/").glob("*/"),
    Path("nazmus_exp/").glob("*/")
)
...
for dataset_path in dataset_paths:
    ...
    df = pd.DataFrame({
        "image_name": [image_name.name for image_name in image_names]
    })
    # frame,time,image_name
    df["frame"] = df.index
    # nazmus_exp/z11c2/images/z11c2--2022-07-22--00-00-01.jpg
    df["time"] = [
        image_name.stem.split("--", 1)[1] for image_name in image_names
    ]
    # convert time to datetime
    df["time"] = pd.to_datetime(df["time"], format="%Y-%m-%d--%H-%M-%S")
    # convert from America/Edmonton to UTC
    df["time"] = df["time"].dt.tz_localize("America/Edmonton").dt.tz_convert("UTC")
