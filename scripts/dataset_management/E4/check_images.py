...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_to_file = {
            executor.submit(calculate_hash, file_path): file_path
            for file_path in second_dir_images
        }
    ...
    first_dir_images = list(first_dir.glob("**/*.png")) + list(
        first_dir.glob("**/*.jpg")
    )
    ...
    for img_path in tqdm(first_dir_images, desc="Processing first directory"):
        ...
        if img_hash in hash_table:
            ...
            # os.remove(img_path)
            logging.info(
                f"Removed duplicate: {img_path} (matches {hash_table[img_hash]})"
            )
            removed_count += 1
        else:
    ...
    logging.info(
        f"Summary: {removed_count} images removed, {not_found_count} images not found in second directory"
    )
    print(
        f"Summary: {removed_count} images removed, {not_found_count} images not found in second directory"
    )
