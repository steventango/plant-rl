...
cost = sum(
    compute_cost(math.ceil(len(job_list) / base_group_size))
...
for path in missing:
    ...
    for g in group(missing[path], groupSize):
        ...
        runner = f"{venv}/.venv/bin/python {cmdline.entry} {gpu_str} -e {path} --save_path {cmdline.results} --checkpoint_path={cmdline.results} -i "
