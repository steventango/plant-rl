...
class GP:
    def __init__(self, input_data, output_data, kernel = gpx.kernels.Matern52(), meanf = gpx.mean_functions.Zero(), key=random.PRNGKey(0)):
        self.kernel = kernel
        self.meanf = meanf
        self.key = key
    ...
    def sample_output(self, x, N = 100):
        ...
        normalized_samples = (random.normal(self.key, (mean.shape[0], N)) * std[:, None]) + mean[:, None]
        normalized_samples = normalized_samples.T
        samples = [sample * self.output_std + self.output_mean for sample in normalized_samples]
        return samples
