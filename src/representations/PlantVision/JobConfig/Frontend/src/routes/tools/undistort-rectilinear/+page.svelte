<script lang="ts">
	...
	async function preview() {
		if (uploaded) {
			...
			// console.log("fx: "+fx);
			let fetchResponse = await fetch('http://localhost:8000/undistort-rectilinear', {
				method: 'POST',
				headers: {
	...
	<div class="grid grid-rows-1 grid-cols-3 gap-5">
		...
		<div class="">
			...
			<div class="grid grid-rows-1 grid-cols-4 pt-20">
				...
				<div class="col-span-2">
					<div class="grid grid-cols-5 grid-rows-1 gap-x-1">
						...
						<input bind:value={p1} class="input" type="text" id="p1" name="p1" placeholder="p1" />
						<input bind:value={p2} class="input" type="text" id="p2" name="p2" placeholder="p2" />
			...
			<div class="text-center pt-20">
				...
				{/if}
				<DownloadJson filename="undistort-rectilinear.json" dataObjToWrite={{
					fx: fx,
					fy: fy,
					cx: cx,
					cy: cy,
		...
		<div class="flex items-center justify-center h-full">
			<div class="grid grid-cols-1 gap-7">
				{#each presets as preset}
					<button class="btn variant-filled mx-3" on:click={() => activatePreset(preset)}>{preset.name}</button>
				{/each}
			</div>
		</div>
	</div>
