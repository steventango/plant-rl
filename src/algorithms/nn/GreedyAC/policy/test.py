...
from algorithms.nn.GreedyAC.policy.MLP import Dirichlet, Gaussian  # noqa: E402
...
def test_dirichlet_vs_gaussian_shapes():
    ...
    dir_action, dir_log_prob, dir_mean = dirichlet_policy.sample(
        state_batch, num_samples=num_samples
    )
    ...
    gauss_action, gauss_log_prob, gauss_mean = gaussian_policy.sample(
        state_batch, num_samples=num_samples
    )
    ...
    dir_action_r, dir_log_prob_r, dir_mean_r = dirichlet_policy.rsample(
        state_batch, num_samples=num_samples
    )
    ...
    gauss_action_r, gauss_log_prob_r, gauss_mean_r = gaussian_policy.rsample(
        state_batch, num_samples=num_samples
    )
    ...
    gauss_actions = (
        torch.rand(batch_size, action_dim) * (action_max - action_min) + action_min
    )
    ...
    dir_action_multi, dir_log_prob_multi, dir_mean_multi = dirichlet_policy.sample(
        state_batch, num_samples=num_samples_multi
    )
    ...
    gauss_action_multi, gauss_log_prob_multi, gauss_mean_multi = gaussian_policy.sample(
        state_batch, num_samples=num_samples_multi
    )
    ...
    print(
        f"  Dirichlet action shape: {dir_action.shape} | Gaussian action shape: {gauss_action.shape}"
    )
    print(
        f"  Dirichlet log_prob shape: {dir_log_prob.shape} | Gaussian log_prob shape: {gauss_log_prob.shape}"
    )
    print(
        f"  Dirichlet mean shape: {dir_mean.shape} | Gaussian mean shape: {gauss_mean.shape}"
    )

    print(f"\nFor multiple samples (num_samples={num_samples_multi}):")
    print(
        f"  Dirichlet action shape: {dir_action_multi.shape} | Gaussian action shape: {gauss_action_multi.shape}"
    )
    print(
        f"  Dirichlet log_prob shape: {dir_log_prob_multi.shape} | Gaussian log_prob shape: {gauss_log_prob_multi.shape}"
    )
    print(
        f"  Dirichlet mean shape: {dir_mean_multi.shape} | Gaussian mean shape: {gauss_mean_multi.shape}"
    )

    print("\nFor log_prob() with action input:")
    print(
        f"  Dirichlet log_prob shape: {dir_log_prob_computed.shape} | Gaussian log_prob shape: {gauss_log_prob_computed.shape}"
    )
