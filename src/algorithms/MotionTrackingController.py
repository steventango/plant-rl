...
class MotionTrackingController(BaseAgent):
    def __init__(self, observations: Tuple[int, ...], actions: int, params: Dict, collector: Collector, seed: int):
        ...
        self.moonlight = 0.35    
        self.c1 = 1.1 - self.moonlight     # determines max intensity, which is self.moonlight + self.c1
    ...
    def start(self, observation: np.ndarray, extra: Dict[str, Any]):   
        ...
        self.mean_clean_areas[self.env_local_time.replace(second=0, microsecond=0)] = float(mean_area)
        ...
        if self.is_night():
            ...
            logger.warning(f"Starting motion-tracking controller in the middle of the day. Enforce standard lighting until tomorrow.")
            action = 1.0
    ...
    def step(self, reward: float, observation: np.ndarray, extra: Dict[str, Any]):
        ...
        if self.is_night():
            ...
            today_morning_time = self.env_local_time.replace(hour=9, minute=0, second=0, microsecond=0)
            today_morning_area = self.mean_clean_areas.get(today_morning_time, 0.0)
            if today_morning_area == 0: 
                logger.warning(f"No same-day morning measurement available at {self.env_local_time}. Enforce standard lighting.")
                action = 1.0
            else: 
