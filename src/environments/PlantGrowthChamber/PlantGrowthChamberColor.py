...
class PlantGrowthChamberColor(PlantGrowthChamber):
    ...
    async def step(self, action: int | float | np.ndarray):
        if isinstance(action, np.ndarray):
            return await super().step(action)

        if isinstance(action, (int, np.integer)) and action in self.action_map:
            return await super().step(self.action_map[action])
