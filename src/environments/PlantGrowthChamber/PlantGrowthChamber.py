...
class PlantGrowthChamber(BaseAsyncEnvironment):
    def __init__(
        ...
        **kwargs,
        ...
        self.image_interval = 10  # minutes between image/power reading
    ...
    async def step(self, action: np.ndarray):
        ...
        if self.last_step_time:
            ...
            elif cycle_time > self.duration + timedelta(seconds=2):
                logger.debug(
                    f"Cycle time ({cycle_time}) exceeded duration {self.duration} by more than 2 seconds)"
                )
