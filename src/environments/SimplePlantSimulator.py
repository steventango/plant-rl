...
class SimplePlantSimulator(BaseEnvironment):
    ...
    def __init__(self, reward_label, **kwargs):
        ...
        self.num_plants = (
            0  # num_plants will be set as the total number of plants in the data
        )
    ...
    def get_observation(self, new_ob=True):
        ...
        if self.day_steps == 0:
            ...
            self.day_steps % self.steps_per_day == 0
        ):  # calculate % growth per overnight step, assuming constant % growth
    ...
    def step(self, action):
        ...
        for pwl in self.actual_areas:
            pwl.insert_plateau(
                self.all_steps, self.all_steps + self.frozen_time(action)
            )
        ...
        if (
            ...
            for pwl in self.actual_areas:
                pwl.insert_plateau(
                    self.all_steps, self.all_steps + self.frozen_time_today
                )
    ...
    def reward_function(self):
        ...
        if self.reward_label == "r1":
            ...
            if (
                self.day_steps % self.steps_per_day == 0
                and floor(self.day_steps / self.steps_per_day) >= 1
            ):
                new_area = np.mean(self.observed_areas[-1])
            ...
            raise ValueError(
                f"{self.reward_label} is an invalid reward_label for the SimplePlantSimulator class."
            )
        ...
        for _ in range(self.num_plants):
            ...
            for i in range(observed_area.shape[0]):
                max_time.append(
                    ...
                    + max_indices[i]
                )  # Let time=0 be at the first time stamp of day 2
            ...
            last_x = (observed_area.shape[0] - 2) * (
                self.steps_per_day + self.steps_per_night
            )
    ...
    def load_area_data(self, time_increment=10):
        ...
        data_path = (
            os.path.dirname(os.path.abspath(__file__)) + "/plant_data/reprocessed.csv"
        )
        ...
        df = full_df[
            (full_df["time_only"] >= start_time) & (full_df["time_only"] <= end_time)
        ]
...
class Daily_Sim(SimplePlantSimulator):
    ...
    def start(self):
        ...
        if self.day_steps % self.steps_per_day == 0 and self.day_steps > 0:
            self.current_state = self.get_observation(
                new_ob=False
            )  # reuse last step of previous episode (morning) as the first step of current episode
        else:
    ...
    def reward_function(self):
        ...
        if self.reward_label == "r1":
            ...
            first_stamp_of_day = (
                floor(self.day_steps / self.steps_per_day) * self.steps_per_day
            )
            ...
            raise ValueError(
                f"{self.reward_label} is an invalid reward_label for the Daily_Sim class."
            )
