...
def run_parallel_requests(image_path, prompts=["cat, dog"], box_threshold=0.3, text_threshold=0.25, server_url="http://localhost:8000/predict"):
    """
    Send multiple requests in parallel with different prompts and collect the results

    Args:
        image_path: Path to the input image
        prompts: List of text prompts to use for each request
        box_threshold, text_threshold, server_url: Parameters for send_request

    Returns:
        List of results from the parallel requests with their corresponding prompts
    """
    n_requests = len(prompts)
    print(f"Sending {n_requests} parallel requests with different prompts...")

    # Create a list of tasks with different prompts
    with concurrent.futures.ThreadPoolExecutor(max_workers=n_requests) as executor:
        # Start the operations and mark each future with its index
        future_to_idx = {
            executor.submit(
                send_request,
                image_path,
        ...
        for future in concurrent.futures.as_completed(future_to_idx):
            ...
            try:
                ...
                print(f"Request with prompt '{prompts[idx]}' completed!")
                # Store both result and prompt used
                results.append((result, prompts[idx]))
            except Exception as exc:
                print(f"Request with prompt '{prompts[idx]}' generated an exception: {exc}")
                results.append((None, prompts[idx]))
...
if __name__ == "__main__":
    ...
    # Run requests with different prompts
    prompts = ["cat, dog", "flower", "tail", "orange", "wall", "plant", "phone", "laptop", "car", "tree"]
    results = run_parallel_requests(
        image_path,
        prompts=prompts,
    ...
    # Process results
    print(f"\nAll {len(results)} parallel requests completed!")

    # Visualize results from each request that succeeded
    for i, (result, prompt) in enumerate(results):
        if result:
            # Create a unique output path for each visualization that includes the prompt
            base_name = os.path.splitext(image_path)[0]
            output_path = f"{base_name}_visualized_{prompt.replace(' ', '_')}.jpg"
            visualize_detections(image_path, result, output_path)
