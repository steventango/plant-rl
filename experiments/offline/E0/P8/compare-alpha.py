...
if __name__ == "__main__":
    ...
    for env, env_df in split_over_column(df, col='environment'):
        for alg, sub_df in split_over_column(env_df, col='algorithm'):
            ...
            ax.plot(np.linspace(0, exp.total_steps, 100), np.ones(100)*121.5, 'k--', linewidth=1, label='return of light-on policy')
            ax.plot(np.linspace(0, exp.total_steps, 100), np.ones(100)*55, 'b--', linewidth=1, label='return of random policy')
