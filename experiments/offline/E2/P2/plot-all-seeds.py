...
def main():
    ...
    for env, env_df in split_over_column(df, col='environment'):
        for alg, alg_df in split_over_column(env_df, col='algorithm'):
            ...
            ax.plot(np.linspace(0, exp.total_steps, 100)*10, np.ones(100)*2.717, 'k-.', linewidth=1, label='light-on')
            ax.plot(np.linspace(0, exp.total_steps, 100)*10, np.ones(100)*1.45, 'b--', linewidth=1, label='random')
