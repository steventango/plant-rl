...
def main():
    ...
    for env, env_df in split_over_column(df, col='environment'):
        ...
        for alg, alg_df in split_over_column(env_df, col='algorithm'):            
            ...
            ax.plot(np.linspace(0, exp.total_steps, 100), np.ones(100)*14.059, 'k-.', linewidth=1, label='light-on')
            ax.plot(np.linspace(0, exp.total_steps, 100), np.ones(100)*3.629, 'b--', linewidth=1, label='random')
