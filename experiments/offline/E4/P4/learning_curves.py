...
optimal_action = np.tile(
    np.hstack([np.ones(12), 2 * np.ones(48), np.ones(12)]), total_days
)[1:-1]
...
def main():
    ...
    for env, env_df in split_over_column(df, col="environment"):
        ...
        for alg, alg_df in split_over_column(env_df, col="algorithm"):  # type: ignore
            sub_df = alg_df
            sub_df = alg_df[(alg_df["environment.reward_period"] == 72)]
        ...
        save(save_path=f"{path}/plots", plot_name=f"{alg}-72-all", save_type="jpg")  # type: ignore
