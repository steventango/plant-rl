    ...
    for env, env_df in split_over_column(df, col='environment'):
        for alg, alg_df in split_over_column(env_df, col='algorithm'):   
            if alg == THIS_AGENT:
                ...
                for i in range(5):
                    ax_r[i].plot(xs[i], ys[i], linewidth=1)
                plt.show()
                ...
                all_data = []
                for i in range(5):
                    ys_w = np.vstack([ys_w1[i][5*steps_per_day:], ys_w0[i][5*steps_per_day:]])
                ...
                for i in range(5):
                    ys_w = np.vstack([ys_w1[i], ys_w0[i]])
                    
                    im = ax[i].imshow(ys_w, aspect=80, extent=(0, ys_w.shape[1], 0, 1), 
