...
THIS_AGENT = 'tc-ESARSA-s1'

STRIDE = {'tc-ESARSA-s1': 1,
          'tc-ESARSA-s2': 2,
          'tc-ESARSA-s3': 3}
...
def last_n_percent_sum(x, n=0.7):
...
def main():
    ...
    for env, env_df in split_over_column(df, col='environment'):
        ...
        for alg, alg_df in split_over_column(env_df, col='algorithm'):
            if alg == THIS_AGENT:
                sub_df = alg_df
                #sub_df = alg_df[alg_df['lambda']==0.9]
                ...
                for i in range(5):
                    ax[i].plot(rescale_time(np.arange(len(optimal_action)), 1), optimal_action, 'r.', label='optimal policy', markersize=0.3)
                    ax[i].plot(rescale_time(xs_a[0], STRIDE[alg]), ys_a[i], 'g.', label=f'seed{i+1}', markersize=0.5)
