...
optimal_action = np.tile(
    np.tile(np.hstack([np.ones(3*6), 2*np.ones(6*6), np.ones(3*6)]), total_days)[1:-1],
    total_episodes
    )


def last_n_percent_sum(x, n=0.1):
    return np.nansum(
        x[:, int((1-n)*x.shape[1]):],
        axis=1
        )
...
def main():
    ...
    for env, env_df in split_over_column(df, col='environment'):
        ...
        for alg, alg_df in split_over_column(env_df, col='algorithm'):
            ...
            xs_a, ys_a = extract_learning_curves(alg_df, report.best_configuration, metric='action', interpolation=None)
            ...
            for i in range(5):
                # Just plot last two weeks
                x_rescaled = rescale_time(xs_a[0], 1)
                x_rescaled -= x_rescaled[-1007]
                ax[i].plot(x_rescaled[-1007:], optimal_action[-1007:], 'r.', label='optimal policy', markersize=0.5)
                ax[i].plot(x_rescaled[-1007:], ys_a[i][-1007:], 'g.', label=f'seed{i+1}', markersize=0.5)
                ax[i].set_ylabel('Action')
                ax[i].set_ylim([-0.1,3.1])
                for j in list(range(total_days + 1)):
                    ax[i].axvline(x = 12*j, color='k', linestyle='--', linewidth=0.5)

            ax[0].set_title(f'Selected actions from final week of {total_days*total_episodes} days')
