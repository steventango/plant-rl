"""Run the E20/P1 Z1-Z5/Z11 deploy configs through the same Problem + Env +
...
We:
  ...
  3. Point `checkpoint_path` / `dataset_npz` at the locally staged
     `checkpoints/E20/P1` tree (run ship_checkpoints.sh first) and push
...
Run:
    python experiments/online/E20/P1/test_via_main_real.py
"""
...
CONFIGS = [
    ("Z1 analytic eval", HERE / "Z1.json"),
    ("Z2 masked eval", HERE / "Z2.json"),
    ("Z3 analytic explore", HERE / "Z3.json"),
    ("Z4 masked explore", HERE / "Z4.json"),
    ("Z5 analytic adaptive", HERE / "Z5.json"),
    ("Z11 masked adaptive", HERE / "Z11.json"),
]
...
def _mocktest_exp(src_path: Path) -> ExperimentModel:
    ...
    mode = "masked" if "masked" in meta["checkpoint_path"] else "analytic"
