"""Run the E22/P1 Z1-Z5 + Z11 deploy configs through the same Problem + Env +
RLGlue path that main_real uses, but against the mock chamber (no wandb, no
hardware, no checkpointing) — purely as a pre-deploy smoke test.

We:
  1. Load each deploy JSON via `ExperimentModel` (same loader main_real uses).
  2. Swap the environment to the mock backend replaying a real 14-day chunk of
     E17 data from `/data/plant-rl/offline/v27/mixed-v27.parquet`, keeping the
     configs' own `log_area` observation and `intensity` action specs.
  3. Point `checkpoint_path` / `dataset_npz` at the locally staged
     `checkpoints/E22/P1` tree (run ship_checkpoints.sh first) and push
     `retrain_after_hour` out of range so AdaptivePPOPolicy's nightly retrain
     stays quiescent (the retrain path is covered by
     tests/algorithms/test_E22P1_parity.py).
  4. Drive `AsyncRLGlue` and render one PDF per zone showing the action
     channels over wall-clock time — the sequence that would reach the lightbar.

Run:
    python experiments/online/E22/P1/test_via_main_real.py
"""
...
CKPT_ROOT = REPO_ROOT / "checkpoints" / "E22" / "P1"
...
CONFIGS = [
    ("Z1 standard exploit", HERE / "Z1.json"),
    ("Z2 differential exploit", HERE / "Z2.json"),
    ("Z3 masked E21-pinned exploit", HERE / "Z3.json"),
    ("Z4 differential explore (adaptive)", HERE / "Z4.json"),
    ("Z5 all_data exploit", HERE / "Z5.json"),
    ("Z11 constant white (control)", HERE / "Constant11.json"),
]
...
def _mocktest_exp(src_path: Path) -> ExperimentModel:
    """Load the deploy JSON, swap to the mock backend, retarget the checkpoint
    paths at the locally staged tree, and return an in-memory ExperimentModel."""
    ...
    # Z11 is a ConstantAgent: no checkpoint to retarget.
    if "checkpoint_path" in meta:
        name = next(
            n
            for n in ("differential", "standard", "masked_e21", "all_data")
            if f"/{n}/" in meta["checkpoint_path"]
        )
        meta["checkpoint_path"] = str(CKPT_ROOT / name / "checkpoint")
        if "dataset_npz" in meta:
            meta["dataset_npz"] = str(CKPT_ROOT / name / "offline_transitions.npz")
        meta["retrain_after_hour"] = 25  # never fires in the mock run
        meta.pop("retrain_archive_dir", None)
...
def _promote_to_six_channel(a_raw) -> np.ndarray:
    ...
    The PPO agents emit scalar s; the env multiplies by BALANCED_ACTION_100.
    Wrapper-enforced (night/flash) actions are already 6-vectors and pass
    through unchanged.
    """
