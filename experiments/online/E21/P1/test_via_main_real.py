"""Run the E21/P1 Z1-Z5/Z11 deploy configs through the same Problem + Env +
...
Run:
    python experiments/online/E21/P1/test_via_main_real.py
"""
...
CKPT_ROOT = REPO_ROOT / "checkpoints" / "E21" / "P1"
...
CONFIGS = [
    ("Z1 analytic exploit", HERE / "Z1.json"),
    ("Z2 masked_log exploit", HERE / "Z2.json"),
    ("Z3 analytic explore (adaptive)", HERE / "Z3.json"),
    ("Z4 masked_log explore (adaptive)", HERE / "Z4.json"),
]
...
def _mocktest_exp(src_path: Path) -> ExperimentModel:
    ...
    mode = "masked_log" if "masked_log" in meta["checkpoint_path"] else "analytic"
    meta["checkpoint_path"] = str(CKPT_ROOT / mode / "checkpoint")
    if "dataset_npz" in meta:
        meta["dataset_npz"] = str(CKPT_ROOT / mode / "offline_transitions.npz")
