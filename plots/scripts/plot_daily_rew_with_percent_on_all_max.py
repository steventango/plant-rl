...
for agent, group in df.groupby('agent'):
    ...
    for i in range(len(daily_groups) - 1):  # Iterate up to the second last day
        ...
        if (next_day - current_day).days == 1:
            ...
            print(f"Current day observations at times {current_max_obs['time'].dt.strftime('%H:%M').values}:")
            print(f"Values: {current_max_obs['mean_clean_area'].values}")
            print(f"Next day observations at times {next_max_obs['time'].dt.strftime('%H:%M').values}:")
            ...
            if not current_max_obs.empty and not next_max_obs.empty:
                current_9am_mean = current_max_obs['mean_clean_area'].mean()
                next_9am_mean = next_max_obs['mean_clean_area'].mean()
