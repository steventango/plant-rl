...
reward_data = []
returns = {
    agent: {} for agent in df["agent"].unique()
}  # Initialize returns for each agent
for agent, group in df.groupby("agent"):
                ...
                reward_data.append(
                    {
                        ...
                        "day": current_day,
                        "day_idx": day_idx,
                        "reward_max_raw": reward_max_raw,
                        "reward_12_raw": reward_12_raw,
                        "reward_9_raw": reward_9_raw,
                        "reward_max_pct": reward_max_pct,
                        "reward_12_pct": reward_12_pct,
                        "reward_9_pct": reward_9_pct,
                    }
                )
...
returns = {
    agent: {
        col: reward_df[
            (reward_df["agent"] == agent) & (reward_df["day_idx"].isin(range(3, 12)))
        ][col].sum()
        for col in reward_df.columns
        if col.startswith("reward")
    }
