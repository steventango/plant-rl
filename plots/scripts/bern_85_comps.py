...
plot_data = df[df['agent'].isin(["Bernoulli p=0.65", "Bernoulli p=0.70", "Bernoulli p=0.85", "Bernoulli p=0.90"])].drop_duplicates(subset=['time', 'agent'])
plot_data['relative_time'] = plot_data.groupby('agent')['time'].transform(lambda x: (x - x.min()).dt.total_seconds() / 3600)
...
plt.gca().xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"day {int(x)//24}"))
plt.xticks(rotation=45)

new_day_rows = plot_data[plot_data['new_day']][['time', 'agent', 'day_idx']].drop_duplicates()
