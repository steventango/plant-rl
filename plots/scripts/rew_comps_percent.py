...
for (day, agent), group in df.groupby(['day', 'agent']):
    reward_max = group['reward_max'].max()  # Get the reward for the last observation of the day
    ...
    if reward_max != 0 and reward_12_avg != 0 and reward_9 != 0:
        plot_data.append({'day': day, 'agent': agent, 'percent_action_1': percent_action_1, 'day_idx': group['day_idx'].iloc[0], 'reward_max': reward_max, 'reward_12_avg': reward_12_avg, 'reward_9': reward_9})
...
titles = ['Rew = ∆ avg of max 5 areas', 'Rew = ∆ avg of 5 areas centered around 12pm', 'Rew = ∆ area at 9:20am']
...
# Add a title above all subplots
fig.suptitle("Reward Comparison by Agent (percent change)", fontsize=16, fontweight='bold')
