...
datasets = []
for p in ['P1']:
    paths = Path("/data/online/E8").joinpath(p).glob("**/z*")
...
def convert_to_edmonton_timezone(utc_time):
    utc_dt = datetime.fromisoformat(utc_time)  # Use fromisoformat to parse ISO 8601 timestamps
...
for agent, group in df.groupby('agent'):
    ...
    unique_days = sorted_group['day'].unique()
    day_to_idx = {day: idx for idx, day in enumerate(unique_days)}
    # Map days to indices for this agent group
    df.loc[group.index, 'day_idx'] = group['day'].map(day_to_idx)
...
for agent, group in df.groupby('agent'):
    ...
    #%%
for target_str in ["09:20", "09:30", "09:40", "09:50", "10:00"]:
    reward_data = []
    for agent, group in df.groupby('agent'):
        ...
        for i in range(len(daily_groups) - 1):  # Iterate up to the second last day
            ...
            if (next_day - current_day).days == 1:
                ...
                current_9am_obs = current_group[current_group['time'].dt.strftime('%H:%M') == target_str]
                next_9am_obs = next_group[next_group['time'].dt.strftime('%H:%M') == target_str]
                ...
                if not current_9am_obs.empty and not next_9am_obs.empty:
                    ...
                    print(f"Current 9am observation: {current_9am_obs.iloc[0]['mean_clean_area']}, Next 9am observation: {next_9am_obs.iloc[0]['mean_clean_area']}")
                    ...
                    reward_data.append({'agent': agent, 'day': current_day, 'reward': diff, 'day_idx': current_group['day_idx'].iloc[0]})
    ...
    for (day, agent), group in df.groupby(['day', 'agent']):
        ...
        percent_action_1 = (group['agent_action'] == 1).mean()  # Calculate percentage of agent_action == 1
        if reward > 0:
            plot_data.append({'day': day, 'agent': agent, 'reward': reward, 'percent_action_1': percent_action_1, 'day_idx': group['day_idx'].iloc[0]})
    ...
    plot_df['day'] = plot_df['day'].astype(str).str.slice(5)  # Extract MM-DD format from the string representation
        ...
        for i, day in enumerate(unique_days):
            ...
            for agent in agents:
                ...
                ax.scatter(agent_data['percent_action_1'], agent_data['reward'], 
                        ...
                        label=agent)                              # Add label for legend
    ...
    fig.legend(by_label.values(), by_label.keys(),
            title="Agent", title_fontsize=14, fontsize=12,
