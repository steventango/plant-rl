...
constant_bright_data = df[df['agent'] == "Constant Bright"].drop_duplicates(subset=['time'])

# Filter for the final 3 days according to day_idx
# max_day_idx = constant_bright_data['day_idx'].max()
# final_days_data = constant_bright_data[constant_bright_data['day_idx'] >= max_day_idx - 2]

# Filter for days 13, 14, and 15
final_days_data = constant_bright_data[constant_bright_data['day_idx'].isin([13, 14, 15, 16])]
...
plt.title("Normalized Area for 'Constant Bright' Agent (Days 13, 14, 15, 16)", fontsize=14)
...
plt.savefig(f"plots/outputs/constant_bright.png", dpi=300)
