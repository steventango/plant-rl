...
ax.set_ylabel('Daily change in area \n (normalized by inital plant size)', rotation=0, labelpad=300, fontsize=40)
...
if num_days > 0:
    for i, day in enumerate(unique_days):
        ...
        for agent in agents:
            ...
            scatter = ax.scatter(agent_data['percent_action_1'], agent_data['reward'],
                                 color=colors[list(agents).index(agent)],
                                 s=70, edgecolor='none', label=agent)
...
fig.text(0.04, 0.5, "Daily change in area (normalized by initial plant size)", va='center', rotation='vertical', fontsize=14)

# Shared legend
fig.legend(legend_handles.values(), legend_handles.keys(), title='Agent', loc='upper center',
           bbox_to_anchor=(0.5, 1.02), ncol=len(agents), fontsize=12, title_fontsize=14)
