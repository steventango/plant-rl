...
reward_cols = ["reward", "reward_20min", "reward_30min", "reward_1hour", "reward_2hour", "reward_4hour", "reward_6hour", "reward_1day"]
for df in [df_1, df_2]:
...
fig, axes = plt.subplots(nrows=len(reward_cols), ncols=1, figsize=(12, 6 * len(reward_cols)), sharex=True)
sns.set_style('whitegrid')
...
for i, reward_col in enumerate(reward_cols):
    ...
    sns.lineplot(data=df_1, x='time', y=reward_col, label=f'Constant Dim - {reward_col}', alpha=0.8, ax=ax)
    sns.lineplot(data=df_2_shifted, x='time', y=reward_col, label=f'Constant Standard (shifted +1 day) - {reward_col}', alpha=0.8, ax=ax)
    ...
    # Add title and labels
    ax.set_title(f'{reward_col} Over Time (Constant Standard Shifted +1 Day)', fontsize=14)
...
#%%
df_1_skip = df_1.copy()
df_1_skip = df_1_skip[df_1_skip["time"] - df_analysis["time"].min() > -pd.Timedelta(minutes=5)]
...
fig, axes = plt.subplots(nrows=n_episodes, ncols=1, figsize=(6, 4 * n_episodes), sharex=True)
