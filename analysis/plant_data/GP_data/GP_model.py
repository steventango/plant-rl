...
class GP:
    def __init__(self, data, kernel = gpx.kernels.RBF(), meanf = gpx.mean_functions.Zero(), action_traces_weights = jnp.array([0.7, 0.5, 0.3]), key=random.PRNGKey(0)):
        self.kernel = kernel
        self.meanf = meanf
    ...
    def preprocess(self, data):
        ...
        for i in range(len(data) -1):
            ...
            trace = self.action_traces_weights * jnp.tile(action, (3, 1)) + (1 - self.action_traces_weights) * trace
            ...
            if data[i, -1].item() != data[i+1, 0].item(): # check if the next point is in sequence with this point
                trace = jnp.zeros((3 , self.trace_2nd_dim))
    ...
    def sample_deltas(self, x, key=random.PRNGKey(0), N=30):
        ...
        normalized_delta_samples = (random.normal(key, (mean.shape[0], N)) * std[:, None]) + mean[:, None]
